...
For a small app, this may be a perfectly suitable way to do things (and likely recommended)

But what happens as our app gets bigger?  We have larger and larger sub component trees? We need to make async calls
to a server to update our todo lists?  You can imagine that the approach we've taken so far may get unwieldy, and makes it
...
that help developers create and maintain a predictable state container for their app.  The basic idea is that any part of the 
application can dispatch "actions" to indirectly manipulate a global app state. Those actions are then use to "compute" the next 
app state using pure functional techniques.  
...
1. Go through the [[https://redux.js.org/][Redux tutorial]] and refactor your app to use the Redux state management pattern.  
